...
def split_data(data, stride, split_comber):
    ...
    xx, yy, zz = np.meshgrid(np.linspace(-0.5, 0.5, int(data.shape[1] / stride)),
                             np.linspace(-0.5, 0.5, int(data.shape[2] / stride)),
                             np.linspace(-0.5, 0.5, int(data.shape[3] / stride)), indexing='ij')
...
def predict_attribute(attribute_net, crop_img):
    attribute_net.eval()
    with torch.no_grad():
        crop_img = Variable(crop_img.cuda(non_blocking=True))
